import {Aes, key, PrivateKey, TransactionBuilder, TransactionHelper} from 'idacjs';
...
const get_fee_list = () => {
    return [
        {id: '1.3.0', precision: 5, symbol: 'IDAC'}
    ];
};
...
const unlock_wallet = (account, password) => {
    return new Promise((resolve, reject) => {
        ...
        console.log('walletPassword', wallet.password_pubkey, password);
...
const import_account_by_password = (account, password) => {
    return import_account(PrivateKey.fromSeed(account + 'owner' + password).toWif(), password);
};
/**
            ...
            return Apis.instance().db_api().exec('get_objects', [account_ids]).then((accounts) => {
                ...
                if (accounts.length > 0) {
                    ...
                    accounts.forEach((account) => {
                        let weight_threshold = account.owner.weight_threshold;
                        // available key should have enough weight
                        let isKeyAvailable = some(account.owner.key_auths, function (key) {
...
const create_account = (account, password) => {
    return new Promise((resolve, reject) => {
        resolve(fetch_dictionary().then(() => {
            let ownerKey = PrivateKey.fromSeed(account + 'owner' + password).toPublicKey().toPublicKeyString();
            let activeKey = PrivateKey.fromSeed(account + 'active' + password).toPublicKey().toPublicKeyString();
            console.log('wifKey', PrivateKey.fromSeed(account + 'owner' + password).toWif(), 'activeKey', activeKey, 'ownerKey', ownerKey);
            return Vue.http.post(`${process.env.faucet_addr}/api/v1/accounts`, {
                name: account,
                owner_key: ownerKey,
                ...
                let wallets = get_wallets();
                let wifKey = PrivateKey.fromSeed(account + 'owner' + password).toWif();
                let password_aes = Aes.fromSeed(password);
                let encryption_buffer = key.get_random_key().toBuffer();
                let encryption_key = password_aes.encryptToHex(encryption_buffer);
                let local_aes_private = Aes.fromSeed(encryption_buffer);
...
/**
 * fetch account balance of IDAC by account name or id
 * @param account_name
 * @returns {bluebird}
 */
const fetch_account_balance = (account_name) => {
    return new Promise((resolve, reject) => {
        resolve(fetch_account(account_name).then((account) => {
            return Apis.instance().db_api().exec('get_account_balances', [account.id, ['1.3.0']]).then(function (balances) {
                return balances && balances.length > 0 ? balances[0] : {amount: 0, asset_id: '1.3.0'};
            });
        }));
    });
};
        ...
        resolve(fetch_account(account_name).then((account) => {
            return Apis.instance().db_api().exec('get_account_balances', [account.id, []]).then(function (balances) {
                ...
                if (balances && balances.length > 0) {
                    // IDAC order first
                    if (balances[1] && balances[1].asset_id === '1.3.0') {
